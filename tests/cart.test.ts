...
describe("Cart", () => {
    ...
    //     cart.addItem("cart-2", "mesa-2", 2.25, 3.75);
    //     cart.addItem("cart-3", "mesa-3", 3.33, 4.67)
    //     addIncrease("discount-1", 10, TypeDiscountEnum.PERCENTAGE, cart);
    //     const totalBeforeDiscount = cart.getCartTotal().getTotal();
    //     addIncrease("discount-2", 5, TypeDiscountEnum.VALUE, cart);
    //     const totalAfterDiscount = cart.getCartTotal().getTotal();
    
    //     expect(totalBeforeDiscount).toBe(totalAfterDiscount);
    //   });
    ...
    it("should add a discount to a specific item in the cart", () => {
        cart.addItem("cart-1", "mesa-1", 1.5, 2.5);
        cart.addItem("cart-2", "mesa-2", 2.25, 3.75);
        cart.addItem("cart-3", "mesa-3", 3.33, 4.67);
        const totalBeforeDiscount = cart.getCartTotal().getTotal();
        addDiscountItem("cart-2", 10, TypeDiscountEnum.FEE, cart);
        const totalAfterDiscount = cart.getCartTotal().getTotal();
